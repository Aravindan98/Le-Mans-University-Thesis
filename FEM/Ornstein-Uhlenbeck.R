...
af<-function(x,theta){return(theta[1]*(theta[2]*gamma(1+1/theta[3])-x))}

bf<-function(x,theta){
  z<-x
  x<-x[x!=0]
  z[z!=0]<-2*theta[1]*theta[2]*gamma(1+1/theta[3])/dweibull(x,scale=theta[2],shape=theta[3])*(pweibull(x,scale=theta[2],shape=theta[3]) - pgamma((x/theta[2])^theta[3],1+1/theta[3]))
  z[z==0]<-0
  return(z)
}

fprime<-function(x,theta){return(theta[3]/theta[2]^2*exp(-(x/theta[2])^theta[3])*((theta[3]-1)*(x/theta[2])^(theta[3]-2)- theta[3]*(x/theta[2])^(2*theta[3]-2)))}

bfprime1<-function(x,theta){return(theta[2]*gamma(1+1/theta[3])-x)}

bfprime2<-function(x,theta){
  z<-x
  x<-x[x!=0]
  z[z!=0]<-fprime(x,theta)/dweibull(x,scale=theta[2],shape=theta[3])^2*(pweibull(x,scale=theta[2],shape=theta[3])- pgamma((x/theta[2])^theta[3],1+1/theta[3]))
  z[z==0]<-theta[2]*(1-1/theta[3])
  return(theta[2]*gamma(1+1/theta[3])*z)
}

bfprime<-function(x,theta){return(2*theta[1]*(bfprime1(x,theta)-bfprime2(x,theta)))}

Weibull.FEM.all<-function(x0,theta,DeltaTime,param=list(Minf=0,Msup=35,N=200),R=10){
